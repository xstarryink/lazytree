...
private:
	...
	{
		for(;mask&&(*cur_node)&&((*cur_node)->key!=invalidKey);mask>>=1)
		{
			cur_node=(*cur_node)->son+static_cast<bool>(key&mask);
		}
...
//			get lazy
			cur_node->reset(new Node{.key=key});
			return cur_node->get();
		}
		else
		{
...
//			expel the occupier
			dynamic_cast<value_t&>(*HalfwayInsert((*cur_node)->key,1<<(highbit((*cur_node)->key)-highbit(key)-1),cur_node))=std::move(dynamic_cast<value_t&>(**cur_node));
			(*cur_node)->key=key;
			return cur_node->get();
		}
	}
